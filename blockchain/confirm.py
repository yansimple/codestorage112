...
def tx_notif(massage):
	...
	mes1 = "Адрес '"+str(wallet)+"' получен успешно, последняя транзакция отслежена, ID транзакции - "+str(tx_id)+"\nОжидайте уведомления!"
	...
	while True:
		try:
			...
			if number_conf >= 2:
				...
				mes_notif = "Получено "+str(number_conf)+" подтверждения на транзакции "+str(tx_id)+" Ваш кошелек '"+str(wallet)+"'\nЕсть вопросы? Напишите @yansimple"
